...
def simular_varias_upas(qtd_upas=3, pessoas_por_upa=10):
    ...
    for fk_upa in range(1, qtd_upas + 1):
        ...
        for p in pessoas:
            ...
            print(f"chegou: {minutos_para_hora(base_horario, p.sala_espera_1_inicio)}")
            print(f"{p.sala_triagem}: {minutos_para_hora(base_horario, p.tempo_triagem_inicio)}")
            print(f"SALA_DE_ESPERA: {minutos_para_hora(base_horario, p.sala_espera_2_inicio)}")
            print(f"{p.sala_consultorio}: {minutos_para_hora(base_horario, p.tempo_atendimento_inicio)}")
            print(f"Saida: {minutos_para_hora(base_horario, p.tempo_saida)}")
